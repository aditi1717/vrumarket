        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-background">
                <div className="text-center">
                    <h2 className="text-2xl font-bold text-textPrimary mb-4">No items to checkout</h2>
        ...
        <div className="bg-background min-h-screen py-4 md:py-12">
            <div className="container mx-auto px-3 md:px-12">
                <div className="flex items-center gap-2 md:gap-4 mb-6 md:mb-10">
                    <button onClick={() => navigate(-1)} className="p-2 -ml-2 hover:bg-background rounded-lg transition-colors text-textPrimary/70">
                        <ArrowLeft size={20} md:size={24} />
                    </button>
                    <div>
                        <h1 className="text-xl md:text-3xl font-black text-textPrimary uppercase tracking-tighter md:tracking-tight leading-none">Checkout</h1>
                        <p className="text-[8px] md:text-[10px] text-textPrimary/55 font-black uppercase tracking-[0.2em] mt-1">Complete your order</p>
                    </div>
                </div>
                ...
                <div className="grid lg:grid-cols-2 gap-12">
                    ...
                    <div className="space-y-8">
                        {/* Shipping Address */}
                        <div className="bg-white p-4 md:p-6 rounded-xl md:rounded-2xl border border-secondary/20 shadow-sm">
                            ...
                            <form id="checkout-form" onSubmit={handleSubmit} className="space-y-3 md:space-y-4">
                                            ...
                                            {userAddresses.map((address, index) => {
                                                ...
                                                return (
                                                    <button
                                                        ...
                                                        className={`text-left border rounded-lg p-3 transition-all ${isSelected ? 'border-primary bg-primary/5' : 'border-secondary/20 hover:border-secondary/35'}`}
                                                    >
                                                        <div className="flex items-center justify-between gap-2">
                                                            <div>
                                                                <p className="text-xs font-bold text-textPrimary">{address.fullName || userData?.name || 'Saved Address'}</p>
                                                                <p className="text-[11px] text-textPrimary/65">{address.phone || userData?.phone || ''}</p>
                                                            </div>
                                                            {address.isDefault && (
                                                        ...
                                                        <p className="text-[11px] text-textPrimary/70 mt-1">
                                ...
                                {userAddresses.length === 0 && (
                                    <p className="text-[11px] text-textPrimary/65 bg-background border border-secondary/20 rounded-lg px-3 py-2">
                                ...
                                {(userAddresses.length === 0 || selectedAddressId === 'manual') ? (
                                    <>
                                        <div className="grid grid-cols-2 gap-3 md:gap-4">
                                            ...
                                            <div className="space-y-1 text-left">
                                                <label className="text-[10px] md:text-xs font-bold text-textPrimary/55 uppercase">Phone</label>
                                                <input
                                                    required
                                                    name="phone"
                                        ...
                                        <div className="space-y-1 text-left">
                                            <label className="text-[10px] md:text-xs font-bold text-textPrimary/55 uppercase">Detailed Address</label>
                                            <textarea
                                                required
                                                name="address"
                                                ...
                                                onChange={handleInputChange}
                                                rows="2"
                                                className="w-full bg-background border border-secondary/20 rounded-lg px-3 md:px-4 py-2 md:py-3 text-sm focus:ring-2 focus:ring-primary/20 outline-none transition-all resize-none"
                                                placeholder="Flat No, Building, Area"
                                            />
                                        </div>
                                        <div className="grid grid-cols-3 gap-2 md:gap-4">
                                            <div className="space-y-1 text-left">
                                                <label className="text-[10px] md:text-xs font-bold text-textPrimary/55 uppercase">City</label>
                                                <input
                                                    required
                                                    name="city"
                                                    value={formData.city}
                                            ...
                                            <div className="space-y-1 text-left">
                                                <label className="text-[10px] md:text-xs font-bold text-textPrimary/55 uppercase">Pincode</label>
                                                <input
                                                    required
                                                    name="pincode"
                                            ...
                                            <div className="space-y-1 text-left">
                                                <label className="text-[10px] md:text-xs font-bold text-textPrimary/55 uppercase">State</label>
                                                <input
                                                    required
                                                    name="state"
                        ...
                        {/* Payment Method */}
                        <div className="bg-white p-4 md:p-6 rounded-xl md:rounded-2xl border border-secondary/20 shadow-sm">
                            <h3 className="text-lg md:text-xl font-bold text-textPrimary mb-4 md:mb-6 flex items-center gap-2">
                                <CreditCard size={18} className="text-primary" />
                                Payment Method
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3">
                                <label className={`flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'cod' ? 'border-primary bg-primary/5' : 'border-secondary/15 hover:border-secondary/20'}`}>
                                    <input
                                        type="radio"
                                        name="payment"
                                        value="cod"
                                    ...
                                    <div className="flex-1">
                                        <div className="font-bold text-textPrimary flex items-center gap-2 text-sm md:text-base">
                                            <Banknote size={14} /> Cash on Delivery
                                        </div>
                                        <div className="text-[10px] md:text-sm text-textPrimary/65">COD Available</div>
                                    </div>
                                </label>

                                <label className={`flex items-center gap-3 md:gap-4 p-3 md:p-4 rounded-xl border-2 cursor-pointer transition-all ${paymentMethod === 'online' ? 'border-primary bg-primary/5' : 'border-secondary/15 hover:border-secondary/20'}`}>
                                    <input
                                        type="radio"
                                        name="payment"
                                        value="online"
                                    ...
                                    <div className="flex-1">
                                        ...
                                        <div className="text-[10px] md:text-sm text-textPrimary/65">Secure Online</div>
                                    </div>
                                </label>
                            </div>
                        </div>
                                ...
                                <div className="space-y-3 md:space-y-4">
                                    <div className="flex gap-2">
                                        <input
                                            ...
                                            className="flex-1 bg-background border border-secondary/20 rounded-lg px-3 py-2 md:py-3 focus:ring-2 focus:ring-primary/20 outline-none transition-all uppercase font-bold text-xs md:text-sm"
                                        />
                                        <button
                                            type="button"
                                            onClick={handleApplyCoupon}
                                            className="bg-secondary text-white px-4 md:px-6 py-2 md:py-3 rounded-lg font-black text-[10px] md:text-xs uppercase tracking-widest hover:bg-primary transition-all shadow-sm"
                                        >
                                            Apply
                                        </button>
                                    </div>
                    ...
                    <div className="h-fit sticky top-28">
                        <div className="bg-white p-5 md:p-8 rounded-2xl md:rounded-3xl border border-secondary/20 shadow-xl relative overflow-hidden">
                            <div className="absolute top-0 left-0 w-full h-1 md:h-2 bg-gradient-to-r from-primary to-secondary" />

                            <h2 className="text-lg md:text-xl font-black text-textPrimary uppercase tracking-tight mb-4 md:mb-6">Order Summary</h2>

                            <div className="max-h-48 md:max-h-60 overflow-y-auto pr-2 space-y-3 md:space-y-4 mb-4 md:mb-6 custom-scrollbar">
                                {enrichedCart.map((item) => (
                                    <div key={item.id} className="flex gap-3 md:gap-4 border-b border-secondary/15 pb-3 last:border-0 last:pb-0">
                                        <div className="w-12 h-12 md:w-16 md:h-16 bg-background rounded-lg overflow-hidden shrink-0">
                                            <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                                        </div>
                                        <div className="flex-1 min-w-0">
                                            <h4 className="text-xs md:text-sm font-bold text-textPrimary line-clamp-2 md:line-clamp-1 truncate-none whitespace-normal leading-tight">{item.name}</h4>
                                            <div className="flex justify-between items-center mt-1">
                                                <div className="flex flex-col">
                                                    <span className="text-[10px] md:text-xs text-textPrimary/55">Qty: {item.qty}</span>
                                                    {item.weight && <span className="text-[10px] text-primary font-bold">{item.weight}</span>}
                                                </div>
                                                <span className="text-xs md:text-sm font-black text-textPrimary">{formatINR(item.price * item.qty)}</span>
                                            </div>
                                        </div>
                                    </div>
                                ))}
                            ...
                            <div className="space-y-3 pt-3 md:pt-4 border-t border-secondary/20">
                                ...
                                <div className="pt-1">
                                    ...
                                    {(shippingQuote.courierName || shippingQuote.error || shippingQuote.source) && (
                                        <div className="text-[9px] md:text-[11px] text-textPrimary/55 mt-2">
                                ...
                                <div className="flex justify-between text-xl md:text-2xl font-black text-primary pt-2 border-t border-secondary/20">
                            ...
                            <button
                                ...
                                className="w-full bg-secondary text-white py-3 md:py-4 rounded-xl font-black text-[11px] md:text-xs uppercase tracking-[0.2em] hover:bg-primary transition-all shadow-lg mt-5 md:mt-8 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 active:scale-95"
                            >
