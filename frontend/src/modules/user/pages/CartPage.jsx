...
const CartPage = () => {
    ...
    if (enrichedCart.length === 0) {
        return (
            <div className="min-h-[40vh] md:min-h-[60vh] flex flex-col items-center justify-center p-4">
                <ShoppingBag className="w-12 h-12 md:w-20 md:h-20 text-secondary/35 mb-4 md:mb-6" />
                <h2 className="text-lg md:text-2xl font-bold text-textPrimary mb-2">Your Bag is Empty</h2>
                <p className="text-sm md:text-base text-textPrimary/65 mb-6 md:mb-8">Add something to your bag and it will show up here.</p>
                <Link to="/catalog" className="bg-primary text-white px-6 py-2 md:px-8 md:py-3 rounded-full font-bold text-sm md:text-base hover:bg-primary transition-all">
                    Shop Now
                </Link>
            </div>
        );
    ...
    return (
        <div className="bg-background min-h-screen py-3 md:py-12">
            <div className="container mx-auto px-2 md:px-12">
                <div className="flex items-center gap-2 mb-3 md:mb-10">
                    <button
                        onClick={() => navigate(-1)}
                        className="w-7 h-7 flex items-center justify-center bg-white border border-secondary/20 rounded-full shadow-sm text-textPrimary/55 hover:text-primary transition-colors"
                    >
                        <ArrowLeft size={16} />
                    </button>
                    <h1 className="text-lg md:text-3xl font-black text-textPrimary uppercase tracking-tight">Shopping Bag</h1>
                </div>
                ...
                <div className="grid lg:grid-cols-3 gap-12">
                        ...
                        {enrichedCart.map((item) => (
                            <div key={item.id} className="bg-white p-2 md:p-6 rounded-lg md:rounded-2xl border border-secondary/20 flex gap-2 md:gap-6 shadow-sm group">
                                <div
                                    ...
                                    className="w-16 h-16 md:w-24 md:h-24 rounded-md md:rounded-xl overflow-hidden bg-background shrink-0 cursor-pointer"
                                >
                                ...
                                <div className="flex-1 flex flex-col justify-between min-w-0">
                                    <div className="flex justify-between items-start">
                                        ...
                                        >
                                            <h3 className="font-bold text-textPrimary text-[13px] md:text-lg group-hover:text-primary transition-colors line-clamp-2 pr-1">{item.name}</h3>
                                            <div className="flex gap-1.5 items-center flex-wrap">
                                                <p className="text-textPrimary/55 text-[9px] md:text-sm uppercase tracking-tighter">{item.category}</p>
                                                {item.weight && (
                                        ...
                                        <div className="flex items-center gap-1">
                                            <button
                                                ...
                                                className="text-textPrimary/40 hover:text-red-500 transition-colors p-1"
                                            >
                                                <Trash2 size={14} />
                                            </button>
                                        </div>
                                    ...
                                    <div className="flex justify-between items-center mt-1 md:mt-4">
                                        <div className="flex items-center border border-secondary/20 rounded-md overflow-hidden bg-background">
                                            <button
                                            ...
                                            <button
                                                ...
                                                className={`p-1 md:p-2 transition-colors ${item.qty >= (item.stock || 0) ? 'text-textPrimary/40 cursor-not-allowed' : 'hover:bg-white'}`}
                                            >
                                                <Plus size={10} />
                                            </button>
                                        </div>
                    ...
                    <div className="h-fit sticky top-28">
                        <div className="bg-white p-4 md:p-8 rounded-xl md:rounded-3xl border border-secondary/20 shadow-sm space-y-3 md:space-y-6">
                            <h2 className="text-base md:text-xl font-black text-textPrimary uppercase tracking-tight">Order Summary</h2>
                            <div className="space-y-2 md:space-y-4 text-[11px] md:text-sm font-medium">
                                <div className="flex justify-between text-textPrimary/65">
                                    <span>Subtotal</span>
                                    <span className="font-bold text-textPrimary">₹{subtotal}</span>
                                </div>

                                <div className="pt-2 md:pt-4 border-t border-secondary/20 flex justify-between text-base md:text-xl font-black text-textPrimary">
                ...
                {enrichedSaved.length > 0 && (
                    <div className="mt-12 border-t border-secondary/20 pt-8">
                        <div className="flex items-center gap-2 mb-6 text-center justify-center">
                            <h3 className="text-lg font-black text-textPrimary uppercase tracking-tight">Reserved for Later</h3>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                            {enrichedSaved.map((item) => (
                                <div key={item.id} className="bg-white p-3 rounded-2xl border border-secondary/15 flex gap-3 shadow-sm hover:shadow-md transition-all h-full">
                                    <div className="w-16 h-16 rounded-xl overflow-hidden bg-background shrink-0">
                                        <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                                    </div>
                                    <div className="flex-1 flex flex-col justify-between min-w-0">
                                        <div>
                                            <h4 className="font-bold text-textPrimary text-xs line-clamp-1">{item.name}</h4>
                                            <div className="flex items-center gap-2 mt-0.5">
                                                <p className="text-[9px] text-textPrimary/55 font-bold">{item.weight || 'Default'}</p>
                ...
                {user && (() => {
                    ...
                    return (
                        <div className="mt-12 md:mt-20 border-t border-secondary/20 pt-10 md:pt-16">
                            <div className="space-y-1 mb-6 md:mb-8 flex items-end justify-between">
                                <div className="text-center md:text-left w-full md:w-auto">
                                    <h3 className="text-lg md:text-xl font-black text-textPrimary uppercase tracking-tight">You Might Also Like</h3>
                                    <p className="text-[9px] md:text-[10px] text-textPrimary/55 font-bold uppercase tracking-[0.2em]">Based on your personality & cart</p>
                                </div>
                            </div>
