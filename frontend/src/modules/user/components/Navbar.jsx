...
import { Search, ShoppingCart, Heart, User, LayoutGrid, Bookmark, ChevronDown, Menu, Home, Bell, Trash2 } from 'lucide-react';
...
const Navbar = () => {
    ...
    const navbarCategories = React.useMemo(
        () => categories.filter((c) => c.status === 'Active' && c.showInNavbar === true),
        [categories]
    );
    ...
    const { filteredProducts, filteredCats, filteredSubs } = React.useMemo(() => {
        ...
        const fc = !selectedCategory ? navbarCategories.filter(c =>
            c.name?.toLowerCase().includes(q)
        ) : [];

        const fs = subCategories.filter(s =>
            s.name?.toLowerCase().includes(q) && s.status === 'Active' && s.showInNavbar !== false
        );

        return { filteredProducts: fp, filteredCats: fc, filteredSubs: fs };
    }, [searchQuery, products, navbarCategories, subCategories, selectedCategory]);
    ...
    return (
        <nav className="bg-background sticky top-0 md:relative border-b border-gray-100 py-2.5 md:py-4 px-4 md:px-12" style={{ zIndex: 10005 }}>
            {/* Desktop View Header */}
            <div className="hidden md:flex justify-between items-center gap-4 md:gap-8">
                ...
                <div className="flex flex-1 max-w-2xl relative group z-50">
                    <div className="flex w-full items-center border border-gray-300 rounded-full bg-background transition-all duration-300 focus-within:border-primary focus-within:ring-1 focus-within:ring-primary/20 hover:border-gray-400 relative">
                        <div
                                    ...
                                    >
                                        ...
                                        <div className="flex-1 bg-white py-4 px-7 border-r border-gray-50">
                                            <div className="grid grid-cols-2 gap-x-4 gap-y-2">
                                                {(hoveredCategoryInMenu || navbarCategories[0]) && (
                                                    (subCategories.filter(s => {
                                                        const hoveredId = hoveredCategoryInMenu?._id || hoveredCategoryInMenu?.id;
                                                        const firstId = navbarCategories[0]?.id || navbarCategories[0]?._id;
                                                        const targetId = hoveredId || firstId;
                                                        const parentId = s.parent?._id || s.parent;
                                                        return String(parentId) === String(targetId) && s.status === 'Active' && s.showInNavbar !== false;
                                                    }).map(sub => (
                                                        <Link
                                                            key={sub.id || sub._id}
                                                            to={getSubLink(sub)}
                                    ...
                                    <div className="flex flex-col gap-1 px-3">
                                        ...
                                        {navbarCategories.map((category) => {
                                            ...
                                            const catSubs = subCategories.filter(s => {
                                                const parentId = s.parent?._id || s.parent;
                                                return String(parentId) === String(category.id || category._id) && s.status === 'Active' && s.showInNavbar !== false;
                                            });
