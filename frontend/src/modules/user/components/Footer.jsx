        ...
        <footer className="bg-[#2C2C2C] text-white pt-10 md:pt-20 pb-10 px-4 md:px-12 relative overflow-hidden border-t border-white/10">
            ...
            <div className="container mx-auto grid grid-cols-2 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-8 mb-16 md:mb-20">
                ...
                <div className="col-span-2 lg:col-span-1 space-y-4 md:space-y-8 mb-4 md:mb-0">
                    ...
                    <div className="flex gap-3 md:gap-4">
                        {config.socials.facebook && (
                            <a href={config.socials.facebook} target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/5 border border-white/20 flex items-center justify-center hover:bg-secondary hover:border-secondary hover:text-white transition-all">
                                <Facebook size={16} md:size={18} />
                            </a>
                        )}
                        {config.socials.instagram && (
                            <a href={config.socials.instagram} target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/5 border border-white/20 flex items-center justify-center hover:bg-secondary hover:border-secondary hover:text-white transition-all">
                                <Instagram size={16} md:size={18} />
                            </a>
                        )}
                        {config.socials.twitter && (
                            <a href={config.socials.twitter} target="_blank" rel="noopener noreferrer" className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-white/5 border border-white/20 flex items-center justify-center hover:bg-secondary hover:border-secondary hover:text-white transition-all">
                ...
                <div className="col-span-2 lg:col-span-1 mt-4 md:mt-0">
                    ...
                    <ul className="space-y-3 md:space-y-6 text-white/70 text-[11px] md:text-sm">
                        <li className="flex gap-3 md:gap-4">
                            <MapPin className="text-secondary shrink-0" size={16} md:size={20} />
                            <span>{config.contact.address}</span>
                        </li>
                        <li className="flex gap-3 md:gap-4">
                            <Phone className="text-secondary shrink-0" size={16} md:size={20} />
                            <span>{config.contact.phone}</span>
                        </li>
                        <li className="flex gap-3 md:gap-4">
                            <Mail className="text-secondary shrink-0" size={16} md:size={20} />
            ...
            {/* Trust Badges */}
            <div className="container mx-auto border-t border-white/10 pt-8 md:pt-10 pb-10 md:pb-20 flex flex-wrap justify-center md:justify-between items-center gap-4 md:gap-8">
                {(config.badges || [
                    ...
                    return (
                        <div key={index} className="flex items-center gap-2 md:gap-3 text-[10px] md:text-sm text-white/70 bg-white/5 px-4 md:px-6 py-2 md:py-3 rounded-xl md:rounded-2xl border border-white/20">
                            <IconComponent className="text-secondary" size={16} md:size={20} />
